...
def load_data():
	...
	if "sentiment" in df.columns:
		# Normalize sentiment text so lowercase CSV values match dashboard filters/metrics.
		df["sentiment"] = df["sentiment"].astype(str).str.strip().str.lower().str.capitalize()
	return df
...
def load_keywords():
	try:
		...
		required_cols = {"keyword", "frequency"}
		if required_cols.issubset(set(keywords_df.columns)):
			return keywords_df
		return pd.DataFrame()
	except Exception:
		return pd.DataFrame()
...
if "sentiment" in df.columns:
	# Build sentiment filter options from dataset values instead of hardcoded labels.
...
product_values = sorted(df["product"].dropna().unique()) if "product" in df.columns else []
...
pos_count = len(filtered_df[filtered_df.get("sentiment") == "Positive"])
neg_count = len(filtered_df[filtered_df.get("sentiment") == "Negative"])
neu_count = len(filtered_df[filtered_df.get("sentiment") == "Neutral"])
...
if not filtered_df.empty and {"product", "sentiment"}.issubset(filtered_df.columns):
	product_sent = filtered_df.groupby("product")["sentiment"].value_counts().unstack(fill_value=0)
	for sentiment in ["Positive", "Negative", "Neutral"]:
		if sentiment not in product_sent.columns:
			product_sent[sentiment] = 0
	product_sent["Total"] = product_sent.sum(axis=1)
	product_sent["Positive %"] = (product_sent["Positive"] / product_sent["Total"] * 100).round(1)
...
if not filtered_df.empty and {"date", "sentiment"}.issubset(filtered_df.columns):
	trend_source = filtered_df.copy()
	trend_source["month"] = trend_source["date"].dt.to_period("M")
	trend = trend_source.groupby(["month", "sentiment"]).size().unstack(fill_value=0)
...
if not keywords_df.empty:
	top_keywords = keywords_df.sort_values("frequency", ascending=False).head(15)
	col_a, col_b = st.columns([3, 2])
	with col_a:
		fig_bar, ax_bar = plt.subplots(figsize=(10, 6))
		ax_bar.barh(top_keywords["keyword"], top_keywords["frequency"], color="skyblue")
